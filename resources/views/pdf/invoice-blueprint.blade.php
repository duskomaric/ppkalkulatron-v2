...
<div class="rail"></div><div class="rail-mark">NACRT / {{ $invoice->invoice_number }}</div>
<div class="page">
    <table class="header"><tr><td class="issuer"><div class="eyebrow">IZDAVALAC</div><div class="company">{{ $company->name }}</div><div class="muted">@if($company->address){{ $company->address }}<br>@endif @if($company->zip || $company->city){{ $company->zip }} {{ $company->city }}<br>@endif @if($company->identification_number)JIB: {{ $company->identification_number }}@endif @if($company->vat_number) · PDV: {{ $company->vat_number }}@endif</div></td><td class="document"><div class="invoice-word">Faktura</div><div class="invoice-number">{{ $invoice->invoice_number }}</div><div class="muted">Datum: {{ $invoice->date?->format('d.m.Y.') ?? '—' }}</div></td></tr></table>
...
<div class="footer">NACRT · {{ $company->name }} · {{ $invoice->invoice_number }}</div>
</body></html>
