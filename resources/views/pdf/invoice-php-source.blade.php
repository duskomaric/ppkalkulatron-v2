...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box }
        body { margin: 0; background: #fff; color: #24292f; font-family: DejaVu Sans Mono, monospace; font-size: 7.2pt; line-height: 1.1 }
        .page { padding: 10mm 12mm 48mm }
        .tabbar table { width: 100%; border-collapse: collapse; border-bottom: 1px solid #d8dce1 }
        .tabbar td { padding: 0; vertical-align: bottom }
        .tab { display: inline-block; padding: 2.2mm 5mm; background: #f3f0fa; border: 1px solid #d8dce1; border-bottom: 0; color: #4b3f7a; font-weight: bold }
        .tab i { color: #9b8fc7; font-style: normal }
        .mark { display: inline-block; padding: 1.4mm 4mm; background: #4f5b93; color: #fff; font-size: 8pt; font-weight: bold; letter-spacing: 1px }
        .src { width: 100%; border-collapse: collapse }
        .src td { padding: 0.15mm 0; vertical-align: top }
        .src .n { width: 11mm; padding-right: 3mm; background: #fafafc; border-right: 1px solid #e6e8eb; color: #b6bbc4; text-align: right }
        .src .c { width: 175mm; padding-left: 4mm }
        .top { margin-top: 4mm }
        .i1 { padding-left: 8mm !important }
        .tg { color: #b31d28; font-weight: bold }
        .kw { color: #6f42c1; font-weight: bold }
        .vr { color: #953800 }
        .st { color: #0a7c42 }
        .nu { color: #0550ae }
        .cm { color: #8b949e }
        ...
        .items th { padding: 1.6mm 1.5mm; background: #f6f4fb; border-top: 1px solid #e6e8eb; border-bottom: 1px solid #dcd7ee; color: #5a4f8a; text-align: left; font-size: 6.2pt; font-weight: normal; text-transform: uppercase; letter-spacing: 0.3px }
        .items td { padding: 2mm 1.5mm; border-bottom: 1px dotted #dfe3e8; vertical-align: top; line-height: 1.3 }
        .items .g { padding-right: 3mm; background: #fafafc; border-right: 1px solid #e6e8eb; color: #b6bbc4; text-align: right; font-size: 7.2pt }
        .r { text-align: right; white-space: nowrap }
        .cn { text-align: center; white-space: nowrap }
        .nm { font-weight: bold }
        .ds { color: #6a737d; font-size: 6.2pt }
        .sum { width: 100%; border-collapse: collapse }
        .sum > tbody > tr > td { padding: 0; vertical-align: top }
        .sum .pad { width: 11mm; background: #fafafc; border-right: 1px solid #e6e8eb }
        .ret { margin: 1.5mm 0 1.5mm 4mm; padding: 2.5mm 4mm; background: #faf9fd; border-left: 3px solid #4f5b93 }
        .rt { width: 100%; border-collapse: collapse }
        .rt td { padding: 1.1mm 0 }
        .rt .v { text-align: right; font-weight: bold }
        .rt .all td { padding-top: 2mm; border-top: 1px solid #ddd8ee; color: #4f5b93; font-size: 10pt; font-weight: bold }
    </style>
</head>
<body>
<div class="page">
    ...
    <table class="src top">
        ...
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="kw">declare</span><span class="pn">(</span><span class="vr">strict_types</span><span class="pn">=</span><span class="nu">1</span><span class="pn">);</span></td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c">&nbsp;</td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="cm">/**</span></td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="cm">&nbsp;* Račun {{ $invoice->invoice_number }} · izdan {{ $invoice->date?->format('d.m.Y.') }}</span></td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="cm">&nbsp;* rok plaćanja {{ $invoice->due_date?->format('d.m.Y.') }} · {{ $invoice->payment_type?->label() }} · valuta {{ $invoice->currency }}</span></td></tr>
        @if($fiscal?->fiscal_invoice_number)
            <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="cm">&nbsp;* fiskalni račun {{ $fiscal->fiscal_invoice_number }}</span></td></tr>
        @endif
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="cm">&nbsp;*/</span></td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c">&nbsp;</td></tr>
        ...
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="pn">];</span></td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c">&nbsp;</td></tr>
        <tr><td class="n">{{ ++$ln }}</td><td class="c"><span class="vr">$stavke</span> <span class="pn">= [</span></td></tr>
    </table>
