{{--
    v1 ovdje ima korisnika i "Odjavi se". v2 nema naloge, pa je umjesto odjave
    zaključavanje — i to samo kad je PIN postavljen.
--}}
...
<x-drawer title="Moj nalog" state="userDrawer">
    <div class="flex items-center gap-4 p-4 bg-[var(--color-surface)] rounded-2xl mb-4 border border-[var(--color-border)] relative overflow-hidden">
        ...
        <div class="z-10">
            <p class="font-black text-base leading-tight italic tracking-tight">ppKalkulatron</p>
            <p class="text-[10px] text-[var(--color-text-muted)] font-bold uppercase tracking-wider mt-0.5">
                {{ $pinEnabled ? 'Zaključavanje uključeno' : 'Bez zaključavanja' }}
            </p>
        </div>
    </div>
    ...
    <div class="space-y-1.5">
        <a href="{{ route('help') }}"
           class="w-full text-left p-3.5 text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] hover:bg-[var(--color-surface-hover)] rounded-xl transition-all flex items-center gap-3 border border-transparent hover:border-[var(--color-border)] group">
        ...
        @if ($pinEnabled)
            <form method="POST" action="{{ route('unlock.destroy') }}">
                ...
                <button type="submit"
                    ...
                    <div class="h-7 w-7 bg-red-500/10 rounded-lg flex items-center justify-center text-red-500">
